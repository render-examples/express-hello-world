...
function writeOfferFixture() {
  const offer = {
    images: { en: 'https://cdn.test/en.png', hi: 'https://cdn.test/hi.png' },
    plans: ['3-Yr Comprehensive'],
    featured: { plan: '3-Yr Comprehensive', price: '₹28,999/year', was: '₹52,499/year', savings: '₹23,500/year' },
    slots: { editable: [{ name: 'language', type: 'text', aliases: ['translate'] }], locked: [] },
  };
  const p = path.join(os.tmpdir(), `offer-${Date.now()}-${Math.random().toString(36).slice(2)}.json`);
...
test('routes a local-source image to the canned flow (no Express API call)', async () => {
  ...
  await actionCreateDesign('phone-r3', { customer: 'Apoorva', model: 'Grand Vitara', plan: '3-Yr Comprehensive' }, { sendImage });

  const reply = await actionEditGraphic('phone-r3', 'local_1', { language: 'Hindi' }, { sendImage });

  assert.match(reply.historyText, /anything else/i);
  assert.equal(sent.at(-1), 'https://cdn.test/hi.png'); // canned Hindi URL — proves the local flow ran
});
